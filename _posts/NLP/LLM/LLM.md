...
- LM은 문장에서 특정 단어의 뒤에 어떤 단어가 나올 확률을 계산하는 것
  - 확률에 의해 추출된 단어를 연결(concat)시켜 새로운 문장을 생성
  - 확률을 계산하기 위해 문장 데이터를 학습시킴
...
- `프롬프트 설계` 및 `프롬프트 최적화`하는 것으로 모델의 연산을 위한 parameter는 변화하지 않음
...
- 사전 훈련이란 모델 제공자가 대형 데이터 셋 미리 학습시킨 모델
- GPT, Llama 등 앞서 소개한 Large Language Model 들이 모두 사전 훈련 모델로 볼 수 있음
- 사전 훈련되지 않은 모델은 모델의 아키텍쳐만 가지고 있는 것 ☞ parameter가 초기 상태
