---
layout: single
title:  'Autoencoder의 모든것의 모든것 (2) Maximum Likelihood 관점의 Deep Learning'
toc: true
...
이로인해 딥러닝에서 사용하는 loss function은 MSE와 Cross Entropy(CE) 뿐이다. 
...
파라미터를 바꿔 loss를 줄이는 것은 알겠으나, gradient descent 방식으로 파라미터를 조정할때 각 파라미터를 얼마만큼 바꿔줘야 하는지에 대한 문제에 마주치게된다. 특히 네트워크가 깊어질수록 파라미터의 차원이 커지게 되므로 명확한 정의가 필요하다. 이에 대한 해답은 다음과 같다.
...
👀 **추가**<br>미분값들은 데이터 포인트(=한 점)에서 미분을 한 것이고, 더 많은 차수를 사용할 수록 approximation error가 작아지게 된다. 

#### ☀️ Learning rate

이때 앞에 있는 learning rate $-n$은 매우 작은 값을 취하게 된다. 이는 앞의 approximation 시에 1차 미분만을 사용하였기 때문이다. 만약 데이터 포인트에서 멀어지는 변화(=learning rate가 클 경우)를 갖게 되면 근사값 오차가 커지게 된다.
...
앞의 내용에서 말했듯 Loss function은 BP를 만족하는 대표적인 두가지인 MSE 또는 CE를 쓴다. 그러면 두 loss 중 어떤것이 좋은가에 대한 문제에 마주치게 되는데, 이때 기준이 되는 두가지 관점이 존재한다. 하나는 Bacpropagation 관점이고 다른 하나는 Maximum Likelyhood 관점이다. 먼저 BP 관점에 대한 해석은 아래와 같다.
...
CE에서는 MSE와 다르게 수식상 activation function의 미분값이 사라지게 된다.
...
가정1. Independence: Training DB 모두에 대한 조건부 확률이 아닌 각 샘플별 조건부 확률의 곱(independence)으로 추정
    ☞ 각 샘플의 확률 곱은 로그 변환으로 인해 각 샘플의 합으로 변경되어 가정 성립함
가정2. Identical Distribution: 각 샘플별 조건부 확률을 모두 동일한(identical) 확률분포를 따른다고 가정
    ☞ loss function이 네트워크의 출력값과 정답하고만 관계있기에 가정 성립함
```

위 가정이 성립함으로 인해 네트워크 출력값을 미리 정해놓은 확률분포의 파라터를 추정하는 것으로 해석할 수 있다.
...
<p align="center"><img src="https://github.com/sigirace/page-images/blob/main/autoencoder/autoencoder1_18.png?raw=true" width="650" height="400"></p>

Maximum Likelihood 관점에서 해석을 할 때, 네트워크의 출력값이 likelihood 값이 아니 확률분포를 규정짓는 파라미터를 추정하는 것이다. 
...
* loss는 backpropagation을 위해 -log를 붙임
* 확률분포가 가우시안이면 파라미터는 평균과 표준편차이고, 수식적으로 계산하면 MSE와 동일함
* Categorical (Multinoulli)인 분포일 경우에도 계산하면 CE와 동일함
...
AE는 입력과 출력이 모두 x로 동일하고, variational autoencoder(VAE)는 주어진 것이 없다. VAE는 training DB에 있는 확률분포를 추정하는 것이다. (후에 상세 설명)
