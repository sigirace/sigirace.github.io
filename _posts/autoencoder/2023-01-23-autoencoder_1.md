...
데이터를 모으는 부분은 고전적 머신러닝과 동일하다. 모델을 정의하는 부분은 역시나 문제에 맞는 딥러닝 모델들을 사용한다. 이때 학습해야 할 파라미터는 주로 네트워크를 구성하는 weight와 bias이다. 이후 딥러닝은 loss function을 정의하는 부분에서 backpropagation으로 인한 제약조건이 생긴다. 이는 backpropagation에 대한 아래 두 가지의 가정 때문이다.
