...
앞선 내용을 통해 추천 시스템의 관심사는 관련(relevant) 아이템을 추천 목록에서 상위에 올리는 것임을 확인하였다. 즉, 추천 시스템은 아래 두가지 목표(추천 순위를 위한 모델링)를 가져야 한다.
...
가장 단순한 방식으로 "relevant한 항목이 첫번째로 나오는 위치"에 대한 metric이다.
